...
describe('E-commerce VercelStore testing', () => {
  it('Should check ability to add products to cart and proceed to payment', function () {
    ...
    cy.url().should('contain', 'demo.vercel.store/search/clothes');
    productsPage.getProductName(products[0].name).should('be.visible').click();
    ...
    cy.url().should('contain', 'demo.vercel.store/search/featured');
    productsPage.getProductName(products[1].name).should('be.visible').click();
    ...
    cy.origin('https://next-js-store.myshopify.com/', { args: { email, firstName, lastName, address, city } }, 
